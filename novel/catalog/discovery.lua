local Explore = require("novel.catalog.explore")
...
function Discovery.isSourceEnabled(source)
    return Capability.canExplore(source)
end

function Discovery.sourceGroups(sources)
    local source_groups, unsupported = {}, {}
    for source_index = 1, #(sources or {}) do
        local source = sources[source_index]
        if Discovery.isSourceEnabled(source) then
            local groups, source_unsupported = Explore.groups(source)
...
function Discovery.canRequestNextPage(source, group, page)
...
function Discovery.run(source, group, page)
    local ok, result = xpcall(function()
        return Explore.run(source, group, {
            page = page,
        })
    end, debug.traceback)
    if ok then
...
function Discovery.encodeResult(result)
...
function Discovery.decodeResult(encoded)
...
function Discovery.runEncoded(source, group, page)
    local ok, encoded_or_error = xpcall(function()
        return Discovery.encodeResult(Discovery.run(source, group, page))
    end, debug.traceback)
    if ok and type(encoded_or_error) == "string" then
        return encoded_or_error
    end
