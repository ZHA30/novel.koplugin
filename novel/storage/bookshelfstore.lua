...
local function normalizeBook(source, book)
    local normalized = BookRecord.new(book)
...
function BookshelfStore:new()
    return setmetatable({
        settings = LuaSettings:open(BookshelfStore.path),
    }, self)
end

function BookshelfStore:list()
...
function BookshelfStore:saveAll(records)
    sortRecords(records)
    self.settings:saveSetting("books", records)
    self.settings:flush()
end

function BookshelfStore:count()
    return #self:list()
end

function BookshelfStore:get(source, book)
...
function BookshelfStore:has(source, book)
    return self:get(source, book) ~= nil
end

function BookshelfStore:add(source, book)
...
function BookshelfStore:applyRefresh(source, book, refresh)
...
function BookshelfStore:clear()
    self:saveAll({})
end

function BookshelfStore.deleteStorage()
    os.remove(BookshelfStore.path)
    os.remove(BookshelfStore.path .. ".old")
end

function BookshelfStore.fetchRefresh(source, book, options)
    options = options or {}
    local BookInfo = options.detail or require("novel.catalog.reading.bookinfo")
    local Toc = options.chapters or require("novel.catalog.reading.toc")
    ...
    local chapters = Toc.run(source, detail.book, {
        refresh = true,
        use_toc_html = false,
        settings = options.settings,
        timeout = options.timeout,
...
function BookshelfStore:refresh(source, book, options)
    local result = BookshelfStore.fetchRefresh(source, book, options)
