...
local function parseBook(source, input_book, rule, response, options)
    ...
    local book = BookRecord.fromSearchBook(input_book)
...
function BookInfo:get(source, search_book, options)
    ...
    if search_book.infoHtml and options.use_info_html ~= false then
        ...
        local spec = RequestSupport.requestSpec(source, search_book.bookUrl, options)
        ...
        local request_response, request_err, failed_response = RequestSupport.execute(self, source, spec)
